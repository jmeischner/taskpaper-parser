...
Parser.prototype.parse = function() {
	...
	preParsed.forEach(function(parsed) {
		if (parsed.type && parsed.type === "project") {
			...
			var projectTags = parsed.tags.map(function(parsedTag) {
				return new Tag(parsedTag.name, parsedTag.value);
			});

			currentProject = new Project(parsed.name, projectTags);
			currentTask = null;
...
Parser.prototype.parseLine = function(line) {
	...
	if (line.match(RegEx.middleColonWithTag)) {
...
Parser.prototype.parseProject = function(line) {
	var projectName = line.split(":")[0].replace(RegEx.startTab, "");
